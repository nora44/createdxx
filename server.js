...
setInterval(() => {
    http.get(`http://alharambotshop2142.glitch.me/`);
}, 280000);
...
● | ${prefix}buy sfa/nfa/net amount -> لشراء حساب
● | ${prefix}stock -> لمعرفة كمية الحسابات المتوفرة
● | ${prefix}add sfa/nfa/net email pass -> لاضافة حساب للبوت
...
const cools = [];
let net = JSON.parse(fs.readFileSync('./nfa.json', 'utf8')); // الملف الي بتحط به حسابات نيتفليكس
...
let NETF = 15000; /*سعر الحساب الواحد النتفيلكيس*/
let SFAP = 4500; /*سعر الحساب الواحد الفل داتا*/
let NFAP = 500; /*سعر الحساب الواحد العادي*/
let URID = '618572047085469697' //مين بيتحوله الكريديت
client.on('message', async message => { 
    ...
    if (message == prefix + 'stock') {
        ...
        message.channel.send(new Discord.RichEmbed().setTitle('💵 PentaGon So 💵') ///1
